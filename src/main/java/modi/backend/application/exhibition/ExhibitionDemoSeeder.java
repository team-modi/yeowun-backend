 ...
 * <ol>
 *   <li>인증키가 있으면 실제 공공데이터 전시를 동기화한다(실패해도 부팅은 계속).</li>
 *   <li>테이블이 비어 있으면 응답 스키마의 <b>모든 필드를 채운</b> 표본 CATALOG + MOCK 전시를 적재한다
 *       (포스터·설명·운영시간·관람료·좌표까지 — 실제 응답처럼 보이게).</li>
 * </ol>
 * 운영/테스트에서는 꺼 둔다(기본 false) — 시드 데이터가 실제 데이터와 섞이지 않게 한다.
 */
@Component
...
public class ExhibitionDemoSeeder implements ApplicationRunner {
	...
	/** 실제 공공데이터 동기화 시도. 인증키가 없거나 외부 장애면 로그만 남기고 넘어간다(데모가 죽지 않게). */
	private void syncRealCatalog() {
		try {
			int synced = exhibitionFacade.syncCatalog();
			log.info("데모 시드: 실제 공공데이터 전시 {}건 동기화", synced);
		} catch (CoreException e) {
			log.warn("데모 시드: 실제 공공데이터 동기화 스킵(외부 불가) — {}", e.getMessage());
		}
	}
	...
	private void seedSamplesIfEmpty() {
		...
		List<Row> rows = List.of(
				new Row("SEED-CCA-204512", "국립현대미술관 소장품 상설전 ‘빛과 그림자’", "국립현대미술관 서울관",
						...
						"https://www.mmca.go.kr/", 126.980781, 37.578608),
				new Row("SEED-CCA-204513", "모네에서 세잔까지 — 인상주의 특별전", "예술의전당 한가람미술관",
						...
						"https://www.sac.or.kr/", 127.011605, 37.479026),
				new Row("SEED-CCA-204514", "서울 도시 사진 아카이브전", "서울시립미술관 서소문본관",
						...
						"https://sema.seoul.go.kr/", 126.973739, 37.564178),
				new Row("SEED-CCA-305001", "부산 현대조각 비엔날레", "부산현대미술관",
						...
						"https://www.busan.go.kr/moca/", 128.887000, 35.106000),
				new Row("SEED-CCA-305002", "경기 미디어아트 페스타", "백남준아트센터",
						...
						"https://njp.ggcf.kr/", 127.108000, 37.256000),
				new Row("SEED-MOCK-0001", "[MOCK] 골목길 드로잉전", "성수동 언노운 갤러리",
						...
						"https://example.com/mock/alley", 127.056000, 37.544000),
				new Row("SEED-MOCK-0002", "[MOCK] 필름 카메라로 담은 대구", "대구예술발전소",
						...
						"https://example.com/mock/daegu", 128.594000, 35.869000),
				new Row("SEED-MOCK-0003", "[MOCK] 종료된 회고전(과거 전시 표본)", "문화역서울284",
						...
						"https://example.com/mock/past", 126.972000, 37.556000));
	...
	private record Row(String externalId, String title, String place, LocalDate startDate, LocalDate endDate,
			...
			String operatingHours, String price, String detailUrl, Double gpsX, Double gpsY) {
	}
}
